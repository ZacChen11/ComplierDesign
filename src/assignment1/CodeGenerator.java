...
public class CodeGenerator {
	...
	public boolean varGenerate(SemRec name, String table, TableHolder t){
		if(!thirdpass){
			return true;
		}
		String[] a = t.search(table, name.rec);
		int size = Integer.parseInt(a[6]);
		//type is int or float
		if(size == 4 ){
			if(a[2].equals("int") || a[2].equals("float")){
				try {
					code_data.write((table+"_"+name.rec+"   dw 0"+'\n').getBytes());
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			try {
				code_data.write((table+"_"+name.rec+"   res "+size+'\n').getBytes());
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public int offsetCompute(SemRec nest, SemRec type, String tablename, TableHolder t, SemRec funname){
		...
		for(int b = 0; b < nest.nest.size(); b++){
			if(b == nest.nest.size()-1){
				type.rec = t.getKind(t2, nest.nest.get(b));
				break;
			}
			t2 = t.getReturnType(t2, nest.nest.get(b));
		}
		if(type.rec.equals("function")){
			...
			for(int i = 0; i < t.tables.size(); i++){
				for(int j = 0; j < t.tables.get(i).entry.size(); j++){
					if(t.tables.get(i).entry.get(j)[0].equals(last)){
						funname.rec = t.tables.get(i).entry.get(j)[4];
						return 0;
					}
				}
			}
		...
		public boolean assGenerate(SemRec nest, SemRec nest2,SemRec right_id, String tablename, TableHolder t){
			if(!thirdpass){
				return true;
			}
			SymbolTable a = t.getTable(tablename);
			...
			//function
			if(type2.rec.equals("function")){
				
				try {
					code_instruction.write(("      jl r15,"+funname2.rec+'\n').getBytes());
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		...
		public boolean func(SemRec funcname, String parentname){
			...
			try {
				code_instruction.write((parentname+":"+funcname.rec+'\n').getBytes());
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		public boolean return_value(SemRec nest, SemRec id, String tablename, String classname, TableHolder t){
			if(!thirdpass){
				return true;
			}
		...
		public void exprcode(SemRec nest, SemRec id, String tablename, TableHolder t){
			...
			//right side is a nest
				...
				if(type.rec.equals("function")){
					...
					try {
						code_instruction.write(("      jl r15,"+funname.rec+'\n').getBytes());
					} catch (IOException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
