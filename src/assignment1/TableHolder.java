...
public class TableHolder {
	...
	public TableHolder(String tableout, String error){
	...
	public boolean duplicateCheck(String Tn, SemRec id){
		...
		for(int a = 0; a < t.entry.size(); a ++){
			if(t.entry.get(a)[0].equals(id.rec)){
				id.rec = id.rec +"*";
			}
		}
		return true;
	...
	public boolean nestType(SemRec type, SemRec nest, String functionname, String classname){
		...
		for(int a = 0; a < nest.nest.size(); a++){
			...
			if(a == nest.nest.size()-1){
				type.rec = getReturnType(table_name, nest.nest.get(a));
				table_name = getReturnType(table_name, nest.nest.get(a));
				return true;
			}
	...
	public boolean funType(SemRec type, String tablename, String classname, TableHolder t){
		if(!secondpass){
			return true;
		}
	...
	public boolean typeCheck(SemRec type1, SemRec type2, int row, int column, Counter c){
		...
		if(type1.rec == null ){
			...
			c.counter = c.counter + 1;
			try {
				ErrorOut.write((type1.rec+" is not equal to "+type2.rec+" [Error location: row:"+row+" column:"+column+"]"+'\n').getBytes());
			} catch (IOException e) {
			// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public int class_var_size(TableHolder t, String tablename){
		SymbolTable a = t.getTable(tablename);
		int size = 0;
		int d_size = 0;
		if(a == null){return 0;};
		for(int b = 0 ; b < a.entry.size(); b++){
			if(a.entry.get(b)[1].equals("variable") && (!a.entry.get(b)[2].equals("int")) && (!a.entry.get(b)[2].equals("float"))){
			...
			if(a.entry.get(b)[1].equals("parameter") && !(a.entry.get(b)[2].equals("int")) && !(a.entry.get(b)[2].equals("float"))){
				if(a.entry.get(b)[3].equals("[]")){
					a.entry.get(b)[6] = Integer.toString(class_var_size(t, a.entry.get(b)[2]));	
				}
				else{
	...
	//update size
	public void update_size(TableHolder t){
		...
		for(int m = 0; m < a.entry.size(); m++){
			...
			if(a.entry.get(m)[0].equals("program") && a.entry.get(m)[1].equals("function")){
